...
fn test_create_and_retrieve_resume_persistence() {
    ...
    let create_response = fixture.client()
        .post("/api/new_resume")
    ...
    let resume_id = created_resume["id"].as_i64().expect("Resume ID should exist") as i32;
    ...
    let get_response = fixture.client()
    ...
    assert_eq!(retrieved_resume["profile_image_url"], "https://example.com/profile.jpg");
    ...
    assert!(retrieved_resume["created_at"].is_string(), "created_at should be present");
    assert!(retrieved_resume["updated_at"].is_string(), "updated_at should be present");
...
fn test_create_resume_appears_in_list() {
    ...
    let unique_email = format!("jane.smith.{}@example.com", 
    ...
    let create_json: Value = serde_json::from_str(&create_body).expect("Valid JSON");
    let created_resume = &create_json["body"]["Resume"];
    let created_id = created_resume["id"].as_i64().expect("Resume ID should exist") as i32;
    
    // Track for cleanup
    fixture.track_resume_id(created_id);
    
    println!("Created resume with ID: {} and email: {}", created_id, unique_email);
    ...
    let found = resumes_array.iter().any(|r| 
        r["email"] == unique_email && 
        r["id"] == created_id
    );
    
    assert!(found, "Newly created resume with ID {} and email {} should appear in the list", 
        created_id, unique_email);

    // Also verify all fields are correct in the list
    let created_resume_in_list = resumes_array.iter()
    ...
    assert_eq!(created_resume_in_list["github_url"], "https://github.com/janesmith");
...
fn test_retrieve_nonexistent_resume() {
    ...
    let message = json["body"]["Message"].as_str().expect("Should have message");
